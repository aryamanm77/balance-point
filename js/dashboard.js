...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.addWaterFromModal = () => {
    addWater();
    addHydrationLogEntry();
  };
  window.startScan = () => startScan();
  window.measureNow = () => {
    closeAllModals();
    setTimeout(() => openModal("modal-scan"), 200);
  };
  window.switchTab = (t) => switchTab(t);
  window.closeAllModals = () => closeAllModals();
  window.medMarkTaken = (id) => {
    medManager == null ? void 0 : medManager.markTaken(id);
    refreshMedsUI();
  };
  window.medRemove = (id) => {
    medManager == null ? void 0 : medManager.removeMed(id);
    refreshMedsUI();
  };
  window.widgetToggle = (id) => widgetDrawer == null ? void 0 : widgetDrawer.toggle(id);
  window.widgetShowAll = () => widgetDrawer == null ? void 0 : widgetDrawer.showAll();
...
function initDate() {
  ...
  if (sdEl) sdEl.textContent = ((_a = dateStr.split(",")[1]) == null ? void 0 : _a.trim()) || dateStr;
  const hour = d.getHours();
  const gSub = document.querySelector(".greeting-sub");
  if (gSub) gSub.textContent = hour < 12 ? "Good morning," : hour < 17 ? "Good afternoon," : "Good evening,";
}
function initMedications() {
  ...
  refreshMedsUI();
  (_a = document.getElementById("med-add-open-btn")) == null ? void 0 : _a.addEventListener("click", () => {
...
function computeEnergyScore() {
  ...
  const activityPct = Math.min(100, stepsVal / 1e4 * 100);
  energyScore = Math.round(sleepScore * 0.4 + medAdh * 0.35 + activityPct * 0.25);
  ...
  if (subEl) {
    subEl.textContent = energyScore >= 80 ? "High \xB7 Keep it up!" : energyScore >= 60 ? "Moderate \xB7 You can do better!" : "Low \xB7 Rest and hydrate";
  }
...
function animateSteps(target) {
  ...
  const timer = setInterval(() => {
    ...
    if (qs) qs.textContent = current >= 1e3 ? (current / 1e3).toFixed(1) + "k" : current;
    if (current >= target) {
      clearInterval(timer);
      computeEnergyScore();
    }
  ...
  setTimeout(() => {
    ...
    const km = document.getElementById("chip-km");
    if (km) km.textContent = (target * 78e-5).toFixed(1);
    const kcal = document.getElementById("chip-kcal");
    if (kcal) kcal.textContent = Math.round(target * 0.04);
    const min = document.getElementById("chip-min");
    if (min) min.textContent = Math.round(target / 100) + " min";
  }, 100);
  buildStepsChart();
}
...
function initModals() {
  ...
  (_a = document.getElementById("nav-scan")) == null ? void 0 : _a.addEventListener("click", () => openModal("modal-scan"));
  document.querySelectorAll("[data-close]").forEach((btn) => {
    btn.addEventListener("click", () => closeModal(btn.dataset.close));
  });
  overlay == null ? void 0 : overlay.addEventListener("click", closeAllModals);
    ...
    mutations.forEach((m) => {
      if (m.type === "attributes" && m.attributeName === "class") {
        ...
        if (el.classList.contains("open")) {
          if (el.id === "modal-sleep") {
            initSleepChart();
            initRestlessnessChart();
          }
          if (el.id === "modal-vitals") {
            initHeartRateChart();
            initSpO2Chart();
          }
          if (el.id === "modal-cardio") {
            initCardioChart();
          }
          if (el.id === "modal-medications") {
            renderMedsModal(medManager);
          }
          if (el.id === "modal-telemetry") {
            setTimeout(() => bedTelemetry == null ? void 0 : bedTelemetry._updateModalGrid(), 200);
          }
        }
      }
    });
...
function openModal(id) {
  var _a, _b;
  closeAllModals();
  (_a = document.getElementById(id)) == null ? void 0 : _a.classList.add("open");
  (_b = document.getElementById("modal-overlay")) == null ? void 0 : _b.classList.add("active");
  document.body.style.overflow = "hidden";
}
function closeModal(id) {
  var _a, _b;
  (_a = document.getElementById(id)) == null ? void 0 : _a.classList.remove("open");
  (_b = document.getElementById("modal-overlay")) == null ? void 0 : _b.classList.remove("active");
  document.body.style.overflow = "";
}
function closeAllModals() {
  var _a;
  document.querySelectorAll(".modal.open").forEach((m) => m.classList.remove("open"));
  (_a = document.getElementById("modal-overlay")) == null ? void 0 : _a.classList.remove("active");
  document.body.style.overflow = "";
}
function initNav() {
...
function switchTab(tab) {
  ...
  (_a = document.getElementById("main-scroll")) == null ? void 0 : _a.scrollTo(0, 0);
}
function initCatBar() {
...
function initSyncAlert() {
  var _a, _b;
  (_a = document.getElementById("sync-dismiss")) == null ? void 0 : _a.addEventListener("click", () => {
  ...
  (_b = document.getElementById("sync-settings")) == null ? void 0 : _b.addEventListener("click", () => openModal("modal-scan"));
}
function initWaterBtn() {
  var _a;
  (_a = document.getElementById("add-water-btn")) == null ? void 0 : _a.addEventListener("click", (e) => {
    e.stopPropagation();
    addWater();
  });
}
function addWater() {
  ...
  updateHydrationUI();
  showToast("\u{1F4A7} 250ml added!", "success");
}
function addHydrationLogEntry() {
  const log = document.getElementById("hydration-log");
  if (!log) return;
  const t = (/* @__PURE__ */ new Date()).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" });
  const item = document.createElement("div");
  item.className = "hl-item";
  item.innerHTML = `<span>\u{1F4A7} ${t}</span><span>250 ml</span>`;
  log.prepend(item);
}
function updateHydrationUI() {
  const pct = Math.round(hydrationMl / HYDRATION_GOAL * 100);
  const lStr = (hydrationMl / 1e3).toFixed(1) + "L";
  ...
  const cur = document.getElementById("hmi-current");
  if (cur) cur.textContent = lStr;
  const mf = document.getElementById("hydration-modal-fill");
  if (mf) mf.style.width = pct + "%";
  const hp = document.getElementById("hydration-pct");
...
function startScan() {
  ...
  const ticker = setInterval(() => {
    dots = (dots + 1) % 4;
    btn.textContent = "Scanning" + ".".repeat(dots);
  }, 400);
  setTimeout(() => {
    ...
    btn.style.background = "linear-gradient(135deg,#22c76c,#00c98d)";
    btn.style.opacity = "1";
    if (result) {
      ...
      result.innerHTML = `
        ...
        <div style="display:flex;gap:16px;justify-content:center;flex-wrap:wrap">
          ...
          <div style="text-align:center"><div style="font-size:22px;font-weight:800;color:#f0f4ff">${tmp}</div><div style="font-size:10px;color:rgba(240,244,255,.5)">\xB0C</div></div>
        </div>`;
      heartRateVal = hr;
      updateHRDisplays(hr);
    }
...
function markTakenLegacy(btn) {
  ...
  if (check) {
    check.className = "med-check taken-check";
    check.textContent = "\u2713";
  }
  btn.remove();
}
