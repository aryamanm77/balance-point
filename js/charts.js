...
function initSleepChart(canvasId = "sleep-chart") {
  const canvas = document.getElementById(canvasId);
  if (!canvas) return;
  destroyChart(canvasId);
  const labels = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
  const data = [6.5, 7.2, 5.8, 8.1, 7, 9.2, 7.7];
  const goalLine = 8;
  const colors = data.map(
    (v) => v >= goalLine ? C.green : v >= 6 ? C.blue : C.orange
  );
...
function initHeartRateChart(canvasId = "hr-chart") {
  const canvas = document.getElementById(canvasId);
  if (!canvas) return;
  destroyChart(canvasId);
...
function initRestlessnessChart(canvasId = "restless-chart") {
  const canvas = document.getElementById(canvasId);
  if (!canvas) return;
  destroyChart(canvasId);
  charts[canvasId] = new Chart(canvas, {
...
function initSpO2Chart(canvasId = "spo2-chart") {
  const canvas = document.getElementById(canvasId);
  if (!canvas) return;
  destroyChart(canvasId);
  const labels = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
  ...
  charts[canvasId] = new Chart(canvas, {
    ...
    options: {
      ...
      scales: {
        ...
        y: {
          grid: { color: C.grid },
          ticks: { color: C.text3, callback: (v) => v + "%" },
          min: 93,
          max: 100
        }
      }
...
function initCardioChart(canvasId = "cardio-week-chart") {
  const canvas = document.getElementById(canvasId);
  if (!canvas) return;
  destroyChart(canvasId);
  const labels = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
  ...
  charts[canvasId] = new Chart(canvas, {
    ...
    data: {
      ...
      datasets: [{
        label: "Cardio Load",
        data,
        backgroundColor: data.map((v, i) => i === 4 ? C.purple : C.purple + "55"),
        borderRadius: 6,
        borderSkipped: false
      }]
    },
